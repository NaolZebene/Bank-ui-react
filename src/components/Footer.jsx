    ...
    <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
                    ...
                    footerLinks.map((link, index) => (
                        <div key={link.key} className="flex flex-col ss:my-0 my-4 min-w-[150px]">
                            ...
                            <ul className={`list-none mt-4`}>
                                {
                                    link.links?.map((one, index) => (
                                        <li key={link.name} className={`font-poppinns font-normal text-[16px] leading-[24px] text-dimWhite hover:text-secondary cursor-pointer ${index != link.links.length - 1 ? "mb-4" : "mb-0"}`}>
                                            {one.name}
                                        </li>
                                    ))
                                }
        ...
        <div className='w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3f3r45]'>
            <p className='font-poppins font-normal text-center text-[18px] leading-[27px] text-white'>
                2021 Naol Zebene. All rights Reserved
            </p>
